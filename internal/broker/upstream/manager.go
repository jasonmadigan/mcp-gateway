...
// expectedProtocolVersion is the version the broker proposes to upstreams
// during initialize. the SDK doesn't export its proposal constant; drift on
// SDK bumps is caught by TestExpectedVersionMatchesSDKProposal.
const expectedProtocolVersion = "2025-11-25"

// GatewayTool pairs a tool definition with the handler the gateway
// registers for it (the SDK has no equivalent of mark3labs' ServerTool).
...
func (a *activeMCP) GetToolHints(served string) (ToolHints, bool) {
	return a.manager.mcp.GetToolHints(served)
}
...
func (man *MCPManager) getTools(ctx context.Context) ([]mcp.Tool, []mcp.Tool, error) {
	...
	// the SDK returns pointer slices; dereference into stable value copies
	...
	return GatewayTool{
		...
		Handler: func(_ context.Context, _ *mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			return &mcp.CallToolResult{
				IsError: true,
				Content: []mcp.Content{&mcp.TextContent{Text: "Kagenti MCP Broker doesn't forward tool calls"}},
			}, nil
		},
	}
}
