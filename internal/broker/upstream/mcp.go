...
// for TLS verification. Headers are injected via a round tripper and a tee
// harvests raw tool annotation hints from tools/list responses.
...
// streamable HTTP client and performs the MCP initialization handshake. If
// already connected, this is a no-op. The initialization result is stored for
// later validation of protocol version and capabilities.
func (up *MCPServer) Connect(ctx context.Context, onConnection func()) error {
	...
	// whenever we do an init store the response for validation and future use
	up.init = session.InitializeResult()

	// register notification and connection-lost handlers after the session is
...
// OnConnectionLost allows registering a connection lost handler.
