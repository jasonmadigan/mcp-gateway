...
func (r *MCPGatewayExtensionReconciler) reconcileActive(ctx context.Context, mcpExt *mcpv1.MCPGatewayExtension) (ctrl.Result, error) {
	// snapshot status before reconcile mutates it (e.g. MCPEndpoint), so updateStatus
	...
	if err := r.checkNamespaceConflict(ctx, mcpExt); err != nil {
		...
		if errors.As(err, &valErr) {
			return ctrl.Result{}, r.updateStatus(ctx, mcpExt, originalStatus, metav1.ConditionFalse, valErr.reason, valErr.message)
		}
		return ctrl.Result{}, err
	}
	...
	if !deploymentReady {
		if err := r.updateStatus(ctx, mcpExt, originalStatus, metav1.ConditionFalse, mcpv1.ConditionReasonDeploymentNotReady, "broker-router deployment is not ready"); err != nil {
			return ctrl.Result{}, err
		}
	...
	return ctrl.Result{}, r.updateStatus(ctx, mcpExt, originalStatus, metav1.ConditionTrue, mcpv1.ConditionReasonSuccess, "successfully verified and configured")
}
...
func (r *MCPGatewayExtensionReconciler) updateStatus(ctx context.Context, mcpExt *mcpv1.MCPGatewayExtension, original *mcpv1.MCPGatewayExtensionStatus, status metav1.ConditionStatus, reason, message string) error {
	...
	// write when anything in status changed, not just the Ready condition:
	// MCPEndpoint is set during reconcile and must be persisted too.
